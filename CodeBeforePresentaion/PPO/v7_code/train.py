...
# Obstacle curriculum — shorter phase 1 so prey encounters cover earlier
CURRICULUM = [
    (0.00, (0, 0)),       # Phase 1: no obstacles (15% of training)
    (0.15, (1, 3)),       # Phase 2: light obstacles (30% of training)
    (0.45, (3, 6)),       # Phase 3: full difficulty (55% of training)
]
...
def main():
    ...
    recent_ep_steps = []   # episode lengths for logging
    ep_rets = [{a: 0.0 for a in agents} for _ in range(NUM_ENVS)]
    prev_phase = None
    t0 = time.time()

    # ---- training dynamics log (saved to JSON at the end) ----
    train_log = []
