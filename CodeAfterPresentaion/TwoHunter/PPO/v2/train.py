"""
PPO training for 2-hunter / 1-prey hunter-prey (v2 — prey rebalance).

v1 (the previous version) trained to 100% capture rate and prey return
locked at -90 by step ~3M, with prey log_std saturated at the +0.5
upper clamp from step 200k onward.  The prey learned essentially
nothing for 9.8M of the 10M steps because all advantages were negative
and small (every episode a loss), so the entropy bonus dominated the
gradient and the policy stayed at maximum-noise sampling.

v2 changes (rebalance prey training, leave hunter logic alone):

  1. SPEED CURRICULUM RE-INTRODUCED (most important change).
     v1 had hunters and prey at 4.5/4.5 throughout, which is a brutally
     unfair starting point — 2v1 at equal speeds with random initial
     policies guarantees the prey loses every episode.  v2 starts
     hunters at 4.0 (SLOWER than prey's 4.5) and ramps to 4.5 = prey
     speed at 50% of training.  Phases:
       0-15%:  hunters 4.0  (prey-favoured, prey LEARNS to escape)
       15-40%: hunters 4.2
       40-70%: hunters 4.4
       70%+:   hunters 4.5  (final equal speeds)
     The early prey-favoured phase is essential — it gives the prey
     positive advantages from successful escapes, which the policy
     gradient can lock onto.  Without this, the prey never accumulates
     the kind of clean training signal that obstacle-use requires.

  2. PREY LOG_STD UPPER CLAMP TIGHTENED +0.5 → +0.0.
     v1 v7 used [-3, +0.5] for both agents, which capped action std at
     exp(0.5) ≈ 1.65.  In 2v1 v1, prey hit this cap by step 200k and
     stayed there for 9.8M steps — the cap was working as a safety net
     but had become the prey's permanent operating point.  Lowering it
     to +0.0 caps std at exp(0) = 1.0, halving the noise headroom and
     forcing the policy to commit to its mean even under weak advantage
     signals.  Hunter clamp UNCHANGED at +0.5 (hunters never saturated).

  3. PREY ENTROPY END COEFFICIENT 0.005 → 0.001.
     With weak advantages, entropy bonus magnitude relative to PG
     gradient determines whether log_std is pushed to the cap.  Lowering
     the end coefficient by 5× reduces this pressure during late
     training.  Combined with the lower clamp (#2) and the curriculum
     (#1), this means by training's end the prey can have a tight,
     committed policy rather than a noise-dominated one.

  4. MAX_STEPS 600 → 450 (env-level change in env.py).  Shorter
     episodes mean more episode completions per update, hence more
     terminal-reward signals available to the prey when escapes do
     happen.  This is NOT a "fairness" change — the per-step game
     dynamics are identical regardless of max_steps.  It's a
     training-signal-density change.
"""
...
# v2: prey end reduced 0.005 → 0.001 to weaken the entropy gradient at the
# clamp boundary so the policy gradient can dominate even with weak advantages.
ENT_COEF = {
    "hunter": {"start": 0.02, "end": 0.003},
    "prey":   {"start": 0.05, "end": 0.001},   # CHANGED end 0.005→0.001
}
VF_COEF        = 0.5
MAX_GRAD_NORM  = 0.5
MINIBATCH_SIZE = 256
...
# v2: per-role log_std clamp bounds.
# Hunter's [-3, +0.5] (std up to ~1.65) unchanged — hunter never saturated.
# Prey's upper bound lowered +0.5 → 0.0 (std up to 1.0).  This forces the
# policy to commit to its mean rather than relying on noise-mode exploration
# at the cap.  See module docstring for full rationale.
LOG_STD_BOUNDS = {
    "hunter": (-3.0, 0.5),
    "prey":   (-3.0, 0.0),    # CHANGED upper bound 0.5 → 0.0
}

HUNTER_FREEZE_FRAC = 0.20

# v2 curriculum: SPEED RAMP RE-INTRODUCED.
# Hunters start slower than prey (4.0 vs 4.5) so the prey wins early
# episodes and develops escape policies before facing equal-speed 2v1.
# (frac_start, obstacle_range, hunter_speed)
CURRICULUM = [
    (0.00, (0, 0), 4.0),     # open field, hunters SLOWER than prey
    (0.15, (1, 3), 4.2),     # sparse obstacles, hunters still slower
    (0.40, (3, 6), 4.4),     # full obstacles, near-equal
    (0.70, (3, 6), 4.5),     # final equal speeds
]
PREY_SPEED = 4.5
