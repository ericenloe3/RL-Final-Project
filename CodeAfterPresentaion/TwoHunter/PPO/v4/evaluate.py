"""
Evaluate trained 2-hunter / 1-prey PPO policies.
...
from train import ActorCritic, RunningNorm, LOG_STD_BOUNDS
...
def load_agent(path, obs_dim, act_dim, hidden, log_std_bounds=(-3.0, 0.5)):
    ckpt = torch.load(path, map_location="cpu", weights_only=False)
    policy = ActorCritic(obs_dim, act_dim, hidden,
                          log_std_min=log_std_bounds[0],
                          log_std_max=log_std_bounds[1])
...
def init_pygame():
    ...
    pygame.display.set_caption("Hunter-Prey PPO (2 hunters)")
...
def draw_frame(screen, font, env, step):
    ...
    hud = font.render(
        ...
        f"H:{env.hunter_speed:.1f}  P:{env.prey_speed:.1f}  obs {len(env.obstacles)}  [PPO 2v1]",
        True, (80, 80, 80),
    )
    screen.blit(hud, (8, 8))
...
def main():
    ...
    for role, path in [("hunter", HUNTER_PATH), ("prey", PREY_PATH)]:
        pol, norm = load_agent(path, obs_dim, act_dim, HIDDEN[role],
                                log_std_bounds=LOG_STD_BOUNDS[role])
        policies[role] = pol; normalizers[role] = norm
        print(f"Loaded {role} from {path}  (hidden={HIDDEN[role]}, log_std bounds={LOG_STD_BOUNDS[role]})")

    print(f"\n{'='*55}")
    print(f"  Phase 1 — Live viewing ({NUM_LIVE} eps)  [PPO 2-hunter]")
    print(f"{'='*55}")
    ...
    for ep in range(1, NUM_LIVE + 1):
        ...
        all_results.append({
            "episode": ep, "phase": "live", "algorithm": "PPO_2hunter",
            "captured": r["captured"],
    ...
    print(f"\n{'='*55}")
    print(f"  Phase 2 — GIF recording ({NUM_GIFS} eps) [PPO 2-hunter]")
    print(f"{'='*55}")
    ...
    for ep in range(1, NUM_GIFS + 1):
        ...
        all_results.append({
            "episode": ep, "phase": "gif", "algorithm": "PPO_2hunter",
