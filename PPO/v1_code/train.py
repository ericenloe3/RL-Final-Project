"""
PPO training for hunter-prey.

Two independent ActorCritic policies, one per agent, trained via PPO.
No shared parameters, no communication — pure independent learning.

Saving: we save FINAL models (not 'best single-agent' checkpoints).
In adversarial co-training, 'best hunter' from step 1M was playing
against a weaker prey from 1M.  Final policies are co-adapted to each
other's current skill, which is what you want for evaluation.
Periodic checkpoints are also saved to inspect training progression.
...
NUM_ENVS        = 16
NUM_STEPS       = 256         # per env per update → batch = 4096
LEARNING_RATE   = 3e-4
GAMMA           = 0.99
GAE_LAMBDA      = 0.95
CLIP_EPS        = 0.2
...
def main():
    ...
    envs    = [HunterPreyEnv() for _ in range(NUM_ENVS)]
