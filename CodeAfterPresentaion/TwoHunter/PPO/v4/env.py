"""
Hunter-Prey environment with TWO hunters (multi-hunter v3).

v3 changes (focused on teaching the prey to actually use obstacles):

  - R_LOS_BREAK reduced 0.15 → 0.05 (transition-only credit, kept small).
  - NEW R_HIDDEN = 0.01 per step per hunter with LOS currently blocked.
    Replaces "you transitioned into hidden" with "you are currently
    hidden", incentivising sustained cover-use rather than rapid LOS
    cycling.
  - NEW R_HIDDEN_BOTH = 0.02 extra per step when BOTH hunter LOSes are
    blocked simultaneously.  Rewards finding the rare double-cover
    positions that genuinely shield from a 2v1 setup.

The total possible per-step prey cover-related reward is now:
    R_HIDDEN × 2 + R_HIDDEN_BOTH = 0.02 + 0.02 = 0.04 per step
which over a 450-step episode is up to +18 — meaningful relative to
the +20 timeout reward and the 100 capture penalty.  Rapid LOS cycling
no longer pays out: each transition gives only R_LOS_BREAK = 0.05 plus
one step of R_HIDDEN, vs the old design's flat 0.15 per cycle.

Earlier versions:
  v1 (multi-hunter base):
    - Two hunter agents, prey captured if either reaches capture_dist.
    - prey_speed raised 4.0 → 4.5 to match hunter speed.
    - max_steps 500 → 600.
    - R_STEP_P 0.02 → 0.03.
  v2 (PPO-side rebalance):
    - max_steps 600 → 450 (faster episode resets).
    - R_LOS_BREAK 0.25 → 0.15 (curb LOS-cycling exploit).
...
Reward design for two hunters:
  ...
  Cover rewards (v3): R_LOS_BREAK = 0.05 transition bonus per hunter,
  PLUS R_HIDDEN = 0.01 per step per hunter currently blocked, PLUS
  R_HIDDEN_BOTH = 0.02 extra when both blocked simultaneously.  This
  rewards SUSTAINED cover (per-step credit) rather than just transitions
  (one-time bonus), which previously incentivised rapid LOS cycling.
...
class HunterPreyEnv:
    ...
    def __init__(
        ...
        prey_speed:   float = 4.5,    # 2v1 default; curriculum ramps hunters from below
        capture_dist: float = 20.0,
        max_steps:    int   = 450,    # CHANGED 600 → 450: more episode resets per
                                       # update means more terminal signals for the
                                       # prey to learn from (see train.py rationale)
        ...
        # LOS-break: split per hunter.  v3: now mostly handled by per-step
        # R_HIDDEN below; keep a small transition bonus for the moment of
        # breaking sight.
        self.R_LOS_BREAK  =   0.05    # CHANGED 0.15 → 0.05 (transition only)

        # v3: per-step hidden bonus.  Active when LOS to a hunter is
        # currently blocked.  Replaces the v2 "transition only" credit
        # with sustained-state credit, rewarding STAYING behind cover
        # rather than rapidly cycling around obstacles for transition
        # bonuses (which v1 trained the prey to game).
        # v4: R_HIDDEN boosted 0.01 → 0.03, R_HIDDEN_BOTH boosted 0.02 → 0.05.
        # Total when fully hidden: 2*0.03 + 0.05 = 0.11/step, exceeding the
        # +0.10/step cap on R_HEADING.  This is the per-step level needed
        # for "stay behind cover" to dominate "run in straight line" in
        # the prey's policy gradient.  Without this magnitude shift the
        # prey learns the easier (but losing) straight-line strategy.
        self.R_HIDDEN       = 0.03    # CHANGED 0.01 → 0.03
        self.R_HIDDEN_BOTH  = 0.05    # CHANGED 0.02 → 0.05
        self.R_COVER_SEEK =   0.03

        # v3-SAC: hunter flanking reward.  Encourages parameter-shared
        # hunters to position on opposite sides of the prey rather than
        # clustering on one side (which they otherwise do because they
        # share a network and have similar observations).
        # Per step per hunter, max +0.02 when flanking perfectly.
        self.R_FLANK      =   0.02
        self.THREAT_RADIUS = 200.0

        # v4: wall penalty redesigned to be GRADED over a large range
        # rather than binary within 40px.  R_WALL_PREY is now the maximum
        # penalty per wall (when prey is exactly at it).  WALL_GRADIENT_RANGE
        # sets where the penalty starts ramping up.  R_CORNER_PREY removed
        # since corner is now naturally larger via quadratic accumulation
        # of multiple wall contributions.
        self.R_WALL_PREY        = -0.05    # per-wall max (was -0.08)
        self.WALL_GRADIENT_RANGE = 150.0   # ramp zone (was 40 binary)
        self.WALL_EDGE          =  40.0    # kept for backward-compat references
    ...
    def step(self, h1_action, h2_action, prey_action):
        ...
        # LOS-break rewards (split per hunter, transition only — small)
        ...
        # v3: per-step "hidden" bonus.  Reward STAYING behind cover, not
        # just transitioning into it.  This is the load-bearing change for
        # teaching the prey to actually use obstacles.
        h1_blocked = los_h1 < 0.5
        h2_blocked = los_h2 < 0.5
        if h1_blocked: rewards["prey"] += self.R_HIDDEN
        if h2_blocked: rewards["prey"] += self.R_HIDDEN
        if h1_blocked and h2_blocked:
            rewards["prey"] += self.R_HIDDEN_BOTH    # double-cover bonus
        ...
        # v3-SAC: smooth search-reward decay rather than hard cutoff at stale=100.
        # Old design: search reward zero at stale >= 100 → flat-valley problem
        # where hunters with no LOS for 100+ steps have NO useful per-step
        # signal and just stop moving (visible in eval as hunters frozen
        # ~600 steps in same positions).
        # New design: linear decay from 1.0 at stale=0 to 0.0 at stale=300.
        # Always *some* gradient toward last-known prey position, never zero.
        SEARCH_DECAY_STEPS = 300
        for hkey, los_val, stale, last_known, h_pos, h_disp_v in [
            ...
            if los_val < 0.5 and stale < SEARCH_DECAY_STEPS:
                decay = max(0.0, 1.0 - stale / SEARCH_DECAY_STEPS)
                ...
                if h_moved > 0.1:
                    ...
                    if lk_dist > 5.0:
                        ...
                        rewards[hkey] += self.R_HEADING * 0.5 * search_app * decay

        # v3-SAC: FLANKING reward.  Encourages the parameter-shared hunters
        # to position on opposite sides of the prey.  Without this signal,
        # both hunters tend to converge to similar action sequences (same
        # network, similar observations) and cluster on one side, leaving
        # the prey free to escape in the other direction.  flank_score in
        # [-1, +1]: +1 = perfect opposite-side flanking, 0 = orthogonal,
        # -1 = same side.  Reward only fires when flank_score > 0.
        if dist_h1 > 1e-6 and dist_h2 > 1e-6:
            v_p_to_h1 = (self.hunter1_pos - self.prey_pos) / dist_h1
            v_p_to_h2 = (self.hunter2_pos - self.prey_pos) / dist_h2
            dot = float(np.dot(v_p_to_h1, v_p_to_h2))
            flank_score = -dot   # +1 when hunters are opposite-side
            if flank_score > 0:
                bonus = self.R_FLANK * flank_score
                rewards["hunter1"] += bonus
                rewards["hunter2"] += bonus
        ...
        # v4: graded over 150px instead of binary at 40px.  Old design
        # gave -0.08 only when within 40px of wall, providing zero
        # gradient at distances > 40px — the prey could happily run
        # toward a wall for 100 steps with no penalty, then suddenly hit
        # the cliff.  New design gives a smooth quadratic ramp from 0
        # (at 150px) to -R_WALL_PREY (at the wall itself), creating
        # gradient pressure away from walls throughout the trajectory.
        # Corner penalty (when within zone of 2 walls) similarly graded.
        px, py = self.prey_pos
        wall_zone = self.WALL_GRADIENT_RANGE
        # Per-wall contribution: 0 outside zone, ramps up quadratically inside
        contributions = []
        for d_to_wall in [px, self.W - px, py, self.H - py]:
            if d_to_wall < wall_zone:
                # Quadratic so the penalty grows faster as wall approaches
                frac = 1.0 - (d_to_wall / wall_zone)
                contributions.append(frac * frac)
            else:
                contributions.append(0.0)
        wall_pressure = sum(contributions)   # 0 to ~4 (all 4 walls is impossible practically)
        rewards["prey"] += self.R_WALL_PREY * wall_pressure
