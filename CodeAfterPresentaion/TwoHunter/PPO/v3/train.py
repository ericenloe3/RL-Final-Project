"""
PPO training for 2-hunter / 1-prey hunter-prey (v3 — teach the prey to hide).

v2 results recap:
  - Curriculum's prey-favoured phase worked: capture stayed near 0% for
    the first 2M steps as designed.
  - But once hunters reached full speed (4.5) by step 7M, capture rate
    collapsed back to 100% and prey return locked at -87.
  - Eval LOS-break counts dropped from v1's huge values (179, 243) to
    single digits — confirming the R_LOS_BREAK reduction worked, but
    revealing the side effect: the prey lost its only "obstacles are
    good" reward signal and stopped using cover at all.
  - Prey log_std was AGAIN saturated at the new upper clamp (+0.0) for
    the entire training run — so the lower clamp also wasn't enough.

v3 changes (env.py + this file):

  ENV.PY (shared with SAC):
    - R_LOS_BREAK reduced 0.15 → 0.05 (transition-only, kept small).
    - NEW R_HIDDEN = 0.01 per step per hunter with LOS blocked.  This is
      the key change: rewards STAYING behind cover instead of just
      transitioning, eliminating the rapid-cycling exploit while
      reinstating the obstacle-use signal.
    - NEW R_HIDDEN_BOTH = 0.02 extra when both LOSes blocked
      simultaneously.  Rewards finding the rare double-cover positions
      that genuinely shield in 2v1.

  TRAIN.PY (PPO-specific):

    1. PREY LOG_STD UPPER CLAMP TIGHTENED FURTHER (0.0 → -0.5).
       v2's 0.0 cap was still saturated.  At -0.5, sampled actions have
       std ≤ exp(-0.5) ≈ 0.6 instead of 1.0, halving the noise floor.
       This lets the policy commit more sharply to its mean even when
       advantages are weak.

    2. PREY END ENTROPY COEFFICIENT 0.001 → 0.0001.
       Effectively zero entropy bonus by training's end.  Combined with
       the tighter clamp, the policy gradient can now dominate.

    3. FINAL HUNTER SPEED 4.5 → 4.4 (curriculum endpoint).
       Restores a 2% prey speed advantage that's small enough to keep
       the contest competitive but large enough to make obstacle-use
       PAY OFF in capture survival.  At 4.5/4.5 a coordinated hunter
       pinch is geometrically uncatchable regardless of cover; at
       4.4/4.5 the prey can escape forming pinches.
"""
...
# v3 curriculum: speed ramp kept, final hunter speed tightened 4.5 → 4.4.
# This restores a small permanent prey advantage (2% speed margin) that
# can be amplified by good cover-use.  At 4.5/4.5 a well-coordinated
# hunter pinch is geometrically uncatchable regardless of obstacles; at
# 4.4/4.5 the prey can escape pinches when they begin to form, making
# the cover-use rewards added in env.py v3 actually pay off in capture
# survival.
...
CURRICULUM = [
    ...
    (0.40, (3, 6), 4.3),     # full obstacles, near-equal
    (0.70, (3, 6), 4.4),     # final: small permanent prey-favoured asymmetry
]
PREY_SPEED = 4.5
