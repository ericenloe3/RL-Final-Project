...
class HunterPreyEnv:
    ...
    def __init__(
        ...
        # Anti-jitter: CLOSING-VELOCITY proximity reward.
        # Rewards the hunter for DECREASING distance when close, not just
        # being close.  Oscillating at fixed distance → Δdist≈0 → 0 reward.
        # Steadily closing → positive Δdist → positive reward.
        self.R_CLOSING_H  =  0.15     # hunter bonus per px of distance closed, within zone
        self.R_CLOSING_P  = -0.15     # prey penalty per px of distance closed against it
    ...
    def obs_size(self) -> int:
        ...
        #   + max_obstacles_obs * 5
        return 18 + self.max_obstacles_obs * 5
    ...
    def step(self, hunter_action, prey_action):
        ...
        # ---- Hunter search reward (when LOS is broken) ----
        # When the hunter can't see the prey, it should move toward the
        # last-known position instead of freezing.  Half-strength heading
        # signal for navigating to where the prey was last seen.
        if curr_los < 0.5 and self._steps_since_los < 100:
            h_moved = float(np.linalg.norm(h_disp))
            if h_moved > 0.1:
                to_last_known = self._h_last_known_prey - self.hunter_pos
                lk_dist = np.linalg.norm(to_last_known)
                if lk_dist > 5.0:
                    to_lk_dir = to_last_known / lk_dist
                    search_approach = float(np.dot(h_disp, to_lk_dir)) / self.hunter_speed
                    rewards["hunter"] += self.R_HEADING * 0.5 * search_approach
        ...
        # ---- Anti-jitter: closing-velocity proximity reward ----
        # Rewards the hunter for DECREASING distance when close.
        # Oscillating at fixed distance → Δdist≈0 → 0 extra reward.
        # Steadily closing in → positive Δdist → bonus.
        # Δdist = prev_dist - dist (positive when closing)
        if dist < self.PROXIMITY_ZONE or prev_dist < self.PROXIMITY_ZONE:
            delta_dist = prev_dist - dist  # positive = hunter closing in
            if delta_dist > 0:
                # Hunter is closing — reward proportional to closure speed
                closing_normed = delta_dist / self.hunter_speed
                rewards["hunter"] += self.R_CLOSING_H * closing_normed
            if delta_dist > 0:
                # Prey let the distance shrink — penalty proportional to closure
                closing_normed = delta_dist / self.hunter_speed
                rewards["prey"] += self.R_CLOSING_P * closing_normed
