# my_game_env.py
"""
'The Most Dangerous Game' — PettingZoo ParallelEnv (v3).

What changed from v2 and why
------------------------------
REMOVED – heading bonus
    dot(action, ideal_dir) sounds helpful but backfires: whenever an obstacle
    sits between the hunter and prey the bonus rewards charging straight into
    the wall.  The hunter then oscillates against the surface and never learns
    to go around.  The progress reward already captures "is the gap closing?"
    without this pathology.

ADDED – wall-proximity penalty
    Prey learned to camp in corners because being wedged against two walls
    halved the number of approach angles.  A smooth penalty based on how close
    each agent is to any boundary discourages this without hard-coding a
    specific strategy.

ADDED – wall distances in observation (4 values)
    Agents can now explicitly see how far they are from each boundary so they
    can factor walls into their planning without needing to infer it from their
    own position alone.

ADDED – line-of-sight flag in observation (1 value)
    A binary feature indicating whether the direct path between the two agents
    is clear.  This gives the hunter a strong signal to "go around" when LOS=0
    and helps the prey distinguish "hunter is close but blocked" from
    "hunter is about to tag me".

ADDED – curriculum support via set_obstacle_range()
    The training script calls this to progressively increase obstacle density
    over the course of training, so agents first master pure pursuit/evasion
    before having to navigate a cluttered map.
...
R_TIMEOUT_PREY     =   10.0
R_STEP_HUNTER      =   -0.05   # time pressure
R_STEP_PREY        =    0.05   # survival bonus
R_PROGRESS_SCALE   =    2.0    # scales (Δdist / AGENT_SPEED) each step
R_OBSTACLE_HIT     =   -2.0    # hunter bumps a wall
...
class MostDangerousGameEnv(ParallelEnv):
    ...
    def step(self, actions: dict):
        ...
        progress   = dist_delta / AGENT_SPEED
