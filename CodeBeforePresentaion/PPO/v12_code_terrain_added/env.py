"""
Hunter-Prey environment — obstacles + terrain effects (v5).

Terrain types (passable regions agents walk through):

  MUD  — the agent's rated speed is multiplied by mud_slow (default 0.5)
         while their centre is inside the zone.  Direction is fully
         controllable.  Symmetric slowdown is strategically interesting:
         the prey may deliberately enter mud to reduce the hunter's speed
         advantage, while the hunter learns to route around or commit.

  ICE  — when an agent's centre enters an ice zone, their action and speed
         at that moment are frozen.  The agent slides at constant velocity
         and CANNOT change direction or speed until their centre exits the
         zone.  The policy's action input is completely ignored while on ice.

         Strategic depth:
           Hunter avoids ice when closing in (loses control).
           Prey can bait the hunter onto ice then break away.
           Prey may ride ice at the right angle to cover distance quickly.

v5 implementation notes:
  1. TerrainZone: passable rect with kind in {"mud", "ice"}.
  2. _last_act records the EFFECTIVE action (frozen heading on ice).  The
     policy always observes its actual movement direction — clean credit signal.
  3. Heading reward normalises by RATED speed (not effective speed).  Mud
     automatically produces lower reward (less displacement) without any
     explicit penalty; agents discover avoidance through the existing signal.
  4. obs_size grows by 4 status scalars + max_terrain_obs*6 zone features.
  5. Curriculum setters: set_mud_range(lo,hi), set_ice_range(lo,hi).
"""
...
class TerrainZone:
    ...
    Agents walk through freely; only their movement parameters change.
    Zones never block pathing (unlike Obstacle).
    """
    __slots__ = ("x", "y", "w", "h", "kind")

    def __init__(self, x, y, w, h, kind):
        self.x, self.y, self.w, self.h = float(x), float(y), float(w), float(h)
        self.kind = kind  # "mud" or "ice"
    ...
    def contains_point(self, px, py):
        """True if (px, py) lies inside this zone (agent centre check)."""
        return (self.x <= px <= self.x + self.w and
                self.y <= py <= self.y + self.h)

    def surface_dist(self, px, py):
        """Distance from point to nearest edge; 0 if already inside."""
        nx = np.clip(px, self.x, self.x + self.w)
        ny = np.clip(py, self.y, self.y + self.h)
        return float(np.sqrt((px - nx)**2 + (py - ny)**2))
...
class HunterPreyEnv:
    ...
    def __init__(
        ...
        # Terrain config — default off; curriculum activates gradually
        n_mud_range:    tuple = (0, 0),
        n_ice_range:    tuple = (0, 0),
        mud_size_range: tuple = (80, 150),
        ice_size_range: tuple = (60, 120),
        mud_slow:       float = 0.5,    # effective speed fraction in mud
        max_terrain_obs: int  = 3,      # K nearest zones in obs vector
    ):
        ...
        # Ice state — per-agent frozen heading
    ...
    def obs_size(self) -> int:
        # base(18) + obstacles(max_obstacles_obs*5)
        #          + terrain_status(4) + terrain_zones(max_terrain_obs*6)
        return 18 + self.max_obstacles_obs * 5 + 4 + self.max_terrain_obs * 6
    ...
    def step(self, hunter_action, prey_action):
        ...
        # ----------------------------------------------------------------
        # 1. Terrain-adjusted action/speed based on CURRENT position
        # ----------------------------------------------------------------
        ...
        # ----------------------------------------------------------------
        # 2. Move (obstacle collision applies normally even on ice/mud)
        # ----------------------------------------------------------------
        ...
        # Record EFFECTIVE action so obs reflects actual heading (important
        # on ice: policy sees frozen direction, not its ignored input)
        self._h_last_act = np.clip(h_eff_act, -1, 1).astype(np.float32)
        self._p_last_act = np.clip(p_eff_act, -1, 1).astype(np.float32)
        self.steps += 1

        # ----------------------------------------------------------------
        # 3. Update ice state from NEW positions (takes effect next step)
        # ----------------------------------------------------------------
        ...
        if h_now_ice and not self._h_on_ice:
            # Entered ice — freeze the action that moved us in
            self._h_on_ice     = True
        ...
        # Heading — normalise by RATED speed so mud automatically yields less
        # reward (smaller displacement) without any extra penalty term.
        if dist > 1e-6:
        ...
        # Cover-seeking (prey, when threatened)
        ...
        if curr_los < 0.5 and self._steps_since_los < 100:
            ...
            if h_moved > 0.1:
                ...
                if lk_dist > 5.0:
                    ...
                    search_app = float(np.dot(h_disp, search_dir)) / self.hunter_speed
        ...
        # Closing-velocity proximity reward
    ...
    def set_mud_range(self, lo, hi):
        """Number of mud zones to spawn each episode."""
        self.n_mud_range = (int(lo), int(hi))

    def set_ice_range(self, lo, hi):
        """Number of ice zones to spawn each episode."""
    ...
    def _resolve_terrain(self, pos, action, rated_speed, on_ice, ice_action):
        """Return (effective_action, effective_speed) for the current terrain.

        Ice takes priority: if already on ice, frozen action/speed are used
        regardless of what other terrain the agent is currently on.
        Mud: reduces speed to mud_slow * rated_speed.
        """
        if on_ice:
            return ice_action, rated_speed   # ice preserves full entry speed
    ...
    def _terrain_at(self, pos):
        """Return first TerrainZone that contains pos, or None."""
    ...
    def _terrain_features(self, agent_pos: np.ndarray) -> np.ndarray:
        """K nearest terrain zones as (rel_cx/W, rel_cy/H, w/W, h/H, dist/DIAG, type).

        type: 0.0 = mud, 1.0 = ice.
        Sorted by surface distance so the most relevant zone is first.
        Padded slots use dist=1.0 (far, mud) as a neutral default.
        """
        ...
        for sd, z in entries:
            feats.extend([
                ...
                z.h / self.H,
                sd / self.DIAG,
                0.0 if z.kind == "mud" else 1.0,  # type encoding
            ])
    ...
    def _obs(self) -> dict:
        ...
        # Terrain status — own state + other agent's state (strategic signal)
        ...
        h_obs = np.concatenate([
            ...
            [h_in_mud, h_in_ice, p_in_mud, p_in_ice],               # 4 terrain status
    ...
    def _spawn_terrain_zones(self):
        """Spawn mud then ice zones, avoiding obstacles and each other.

        Terrain zones are intentionally large (60-150 px) to create meaningful
        strategic areas.  They cannot overlap solid obstacles (visual clarity
        and to prevent zones that are unreachable or confusing to navigate).
        """
        zones  = []
        ...
        for kind, (lo, hi), size_range in [
            ...
            while placed < n and attempts < 300:
                ...
                # Don't cover any solid obstacle
                if any(self._rects_overlap(cand, o, pad=5) for o in self.obstacles):
                    continue
                # Don't overlap other terrain zones
    ...
    def _rects_overlap(a, b, pad=0.0) -> bool:
        """Works for any objects with .x .y .w .h (Obstacle or TerrainZone)."""
        return not (
    ...
    def _move(self, pos, action, speed):
        """Move at effective speed.  Obstacle collision still applies on ice/mud."""
    ...
    def _line_of_sight(self):
        """LOS blocked only by solid obstacles, not terrain zones."""
        if not self.obstacles:
            return 1.0
