...
GAE_LAMBDA      = 0.95
CLIP_EPS        = 0.2
ENT_COEF_START  = 0.02        # higher early → more exploration of dodge strategies
ENT_COEF_END    = 0.005       # lower late → sharp policy
VF_COEF         = 0.5
MAX_GRAD_NORM   = 0.5
UPDATE_EPOCHS   = 4
MINIBATCH_SIZE  = 256
...
# Phase 1: no obstacles (master pursuit/evasion + learn wall avoidance)
...
def main():
    ...
    for update in range(1, num_updates + 1):
        ...
        # ---- PPO updates ----
        frac = global_step / TOTAL_TIMESTEPS
        ent_coef = anneal(ENT_COEF_START, ENT_COEF_END, frac)
        ...
        for a in agents:
            ...
            logs[a] = ppo_update(policies[a], optimizers[a], b_obs, b_act, b_lp, b_adv, b_ret, ent_coef)
        ...
        if update % LOG_EVERY == 0:
            ...
            print(f"upd {update:>5}/{num_updates}  step {global_step:>9,}  {sps:>5.0f} sps  "
                  f"eps {ep_count:>6,}  cap {cap:>5.1f}%  obs {obs_range}  ent_c {ent_coef:.4f}")
