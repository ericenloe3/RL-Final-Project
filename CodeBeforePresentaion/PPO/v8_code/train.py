...
Changes from v1:
  ...
  - Obs size grew from 7 → 37 (LOS + wall distances + 5 nearest obstacles).
    Hidden size bumped 64 → 128 to accommodate the larger input.
  - Training dynamics saved to models/train_log.json for post-processing.
...
GAE_LAMBDA      = 0.95
CLIP_EPS        = 0.2
# Per-agent entropy: prey needs MORE exploration to discover dodging strategies.
# Hunter converges fast; prey needs to keep exploring.
ENT_COEF = {
    "hunter": {"start": 0.02, "end": 0.003},
    "prey":   {"start": 0.05, "end": 0.01},    # 2.5× higher than hunter
}
VF_COEF         = 0.5
MAX_GRAD_NORM   = 0.5
UPDATE_EPOCHS   = 4
...
# Combined curriculum: obstacles AND hunter speed ramp up together.
# The prey needs to EXPERIENCE escapes early, then face increasing difficulty.
# (frac_of_training, obstacle_range, hunter_speed)
PREY_SPEED = 4.0   # constant throughout
CURRICULUM = [
    # Phase 1: EQUAL speeds, no obstacles → prey escapes regularly,
    # learns "surviving = good", builds evasion fundamentals
    (0.00, (0, 0), 4.0),
    # Phase 2: slight hunter edge, light obstacles → prey must start
    # using cover to escape; still escapes ~30-50% of the time
    (0.15, (1, 3), 4.2),
    # Phase 3: moderate edge, full obstacles → prey needs tactical play
    (0.40, (3, 6), 4.3),
    # Phase 4: final difficulty — hunter is clearly faster
    (0.70, (3, 6), 4.5),
]
...
def ppo_update(policy, optimizer, obs, acts, old_lp, advs, rets, ent_coef):
...
def main():
    ...
    for update in range(1, num_updates + 1):
        ...
        for a in agents:
            ...
            logs[a] = ppo_update(policies[a], optimizers[a], b_obs, b_act, b_lp, b_adv, b_ret, ent_coefs[a])
        ...
        if update % LOG_EVERY == 0:
            ...
            print(f"upd {update:>5}/{num_updates}  step {global_step:>9,}  {sps:>5.0f} sps  "
                  ...
                  f"h_spd {h_speed:.1f}  "
                  f"ent h:{ent_coefs['hunter']:.3f} p:{ent_coefs['prey']:.3f}")
