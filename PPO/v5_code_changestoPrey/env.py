...
class HunterPreyEnv:
    ...
    def __init__(
        ...
        # Anti-wall-camping (v4)
        self.R_WALL_PREY  =  -0.02    # tiny penalty, prey only, within WALL_EDGE of boundary
        self.WALL_EDGE    =  15.0     # pixels — very thin strip at screen edge
    ...
    def reset(self, seed=None) -> dict:
        ...
        # 3. Spawn prey away from walls (90px padding) so early training
        #    episodes are open-space evasion, not "immediately hit a wall"
    ...
    def step(self, hunter_action, prey_action):
        ...
        if dist > 1e-6:
            ...
            # Displacement-scaled heading: multiply by actual_speed / rated_speed.
            # A wall-stuck agent gets 0 heading reward even if its action points
            # away from the hunter — clean gradient saying "walls reduce reward."
            h_speed_frac = min(1.0, h_moved / self.hunter_speed)
            p_speed_frac = min(1.0, p_moved / self.prey_speed)

            h_approach = float(np.dot(h_disp, to_prey)) / max(h_moved, 1e-8)  # normalise by actual disp
            p_escape   = float(np.dot(p_disp, to_prey)) / max(p_moved, 1e-8)
            rewards["hunter"] += self.R_HEADING * h_approach * h_speed_frac
            rewards["prey"]   += self.R_HEADING * p_escape   * p_speed_frac
        ...
        # ---- Prey wall penalty (v4) — tiny, only at screen edge ----
        prey_wall_d = min(
            self.prey_pos[0],
            self.W - self.prey_pos[0],
            self.prey_pos[1],
            self.H - self.prey_pos[1],
        )
        if prey_wall_d < self.WALL_EDGE:
            rewards["prey"] += self.R_WALL_PREY  # -0.02 flat penalty at edge
