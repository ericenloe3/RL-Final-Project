...
Differences from the single-hunter v5 SAC env:
  ...
  - prey_speed default raised 4.0 → 4.5 (matches hunter).  In 1v1 the
    ...
    speeds at the end of training keeps the contest from becoming
    trivially hunter-dominated.  The curriculum (in train.py) starts
    hunters slower than prey and ramps up to give the prey a winning
    environment during early training.
  - max_steps default 500 → 450.  Shorter than the original 600 (which
    we tried first and found too long): with 100% capture rate the prey
    was getting cut off at ~130 steps anyway, so max_steps wasn't the
    binding constraint — but lowering it produces faster episode
    completion which accelerates the prey's training-signal accumulation
    when escape episodes do occur.
...
class HunterPreyEnv:
    ...
    def __init__(
        ...
        # LOS-break: split per hunter.  Total per step possible 0.5 → 0.3
        # (v2): in 2v1 the prey could rack up huge LOS-break rewards by
        # cycling rapidly around obstacles, which trained it to game the
        # signal rather than execute genuine escape.  Lower coefficient
        # keeps the cover-seeking gradient but de-emphasises rapid cycling.
        self.R_LOS_BREAK  =   0.15
