...
R_OBSTACLE_HIT     =   -0.5    # hunter bumps obstacle
R_OBSTACLE_HIT_PREY=   -1.0    # prey bumps obstacle
# Wall penalties — prey penalty is much larger so it can't profitably camp a corner.
R_WALL_PENALTY_HUNTER =  0.8   # max per-step penalty for hunter hugging screen edge
R_WALL_PENALTY_PREY   =  3.0   # max per-step penalty for prey  hugging screen edge
WALL_MARGIN        =   80      # pixels — inside this zone penalty ramps up (was 60)
...
# Stuck penalty: per step, for each step the agent hasn't moved appreciably.
# Escalates linearly so a genuinely wedged agent is strongly incentivised to escape.
R_STUCK_SCALE      =   0.15    # reward penalty per stuck-step (applied each step)
STUCK_THRESH_PX    =   0.3     # movement below this (px) counts as "stuck"
MAX_STUCK_OBS      =   30      # normalisation ceiling for the obs feature
...
class MostDangerousGameEnv(ParallelEnv):
    ...
    def __init__(self, render_mode=None):
        ...
        self.timestep = 0
        self.hunter_stuck_steps: int = 0
        self.prey_stuck_steps:   int = 0

        # obs = 2+2+1+2+4+1+max_obstacles*5 + 2 stuck counters = 64
        self._obs_dim = 2 + 2 + 1 + 2 + 4 + 1 + self.max_obstacles * 5 + 2
    ...
    def _get_obs(self) -> dict[str, np.ndarray]:
        ...
        # Stuck counters (normalised to [0, 1]) appended last so existing
        # obstacle features are untouched.
        hunter_stuck_n = np.float32(min(self.hunter_stuck_steps, MAX_STUCK_OBS) / MAX_STUCK_OBS)
        prey_stuck_n   = np.float32(min(self.prey_stuck_steps,   MAX_STUCK_OBS) / MAX_STUCK_OBS)

        hunter_obs = np.append(hunter_obs, [hunter_stuck_n, prey_stuck_n])
        prey_obs   = np.append(prey_obs,   [prey_stuck_n,   hunter_stuck_n])

        return {"hunter": hunter_obs.astype(np.float32), "prey": prey_obs.astype(np.float32)}
    ...
    def reset(self, seed=None, options=None):
        ...
        self.hunter_stuck_steps  = 0
        self.prey_stuck_steps    = 0
    ...
    # Eight evenly-spaced unit escape directions (cached at class level).
    _ESCAPE_ANGLES = np.linspace(0, 2 * np.pi, 9)[:-1]   # 0°,45°,…,315°
    _ESCAPE_DIRS   = np.stack([np.cos(_ESCAPE_ANGLES),
                                np.sin(_ESCAPE_ANGLES)], axis=1).astype(np.float32)

    def _try_move(self, pos: np.ndarray, action: np.ndarray, speed: float):
        """Move pos by action*speed with sliding + escape-nudge collision response.

        Priority:
          1. Full move       -- accepted if clear.
          2. X-only slide    -- slide along vertical obstacle face.
          3. Y-only slide    -- slide along horizontal obstacle face.
          4. Escape nudge    -- truly cornered; try 8 directions sorted by
                               closeness to the intended action and take the
                               first free one.  Uses half-speed so the nudge
                               is subtle.  This prevents agents from freezing
                               against a corner of two obstacles or a wall
                               meeting an obstacle.
          5. No move         -- all 8 escape directions blocked (rare).
        """
        delta = action * speed
        ...
        # 4. Escape nudge -- sort 8 directions by dot-product with intended action
        #    so the agent moves as close to its desired direction as possible.
        action_norm = action / (np.linalg.norm(action) + 1e-8)
        order = np.argsort(-self._ESCAPE_DIRS @ action_norm)
        nudge_dist = speed * 0.5
        for i in order:
            candidate = clamp(pos + self._ESCAPE_DIRS[i] * nudge_dist)
            if not self._collides(candidate):
                return candidate, True

        # 5. Genuinely stuck (extremely rare -- e.g. spawned inside geometry)
        return pos.copy(), True

    def _wall_penalty(self, pos: np.ndarray, scale: float) -> float:
        """Smooth penalty that ramps from 0 at WALL_MARGIN to `scale` at the edge."""
        d = min(
            pos[0],
            self._W - pos[0],
            pos[1],
        ...
        if d >= WALL_MARGIN:
            return 0.0
        return scale * (1.0 - d / WALL_MARGIN)
    ...
    def step(self, actions: dict):
        ...
        prev_hunter_pos = self.hunter_pos.copy()
        prev_prey_pos   = self.prey_pos.copy()
        ...
        # ---- Track stuck steps ---------------------------------------
        # An agent is "stuck" if its net displacement this step is tiny.
        # The counter is used both as an observation feature and to compute
        # an escalating penalty that incentivises the agent to break free.
        h_moved = float(np.linalg.norm(self.hunter_pos - prev_hunter_pos))
        p_moved = float(np.linalg.norm(self.prey_pos   - prev_prey_pos))
        self.hunter_stuck_steps = 0 if h_moved > STUCK_THRESH_PX else self.hunter_stuck_steps + 1
        self.prey_stuck_steps   = 0 if p_moved > STUCK_THRESH_PX else self.prey_stuck_steps   + 1
        ...
        # ---- Wall-proximity penalty (separate scales per agent) ------
        # Prey penalty is much larger so camping at a corner is not profitable.
        rewards["hunter"] -= self._wall_penalty(self.hunter_pos, R_WALL_PENALTY_HUNTER)
        rewards["prey"]   -= self._wall_penalty(self.prey_pos,   R_WALL_PENALTY_PREY)

        # ---- Stuck penalty (escalates linearly with stuck duration) --
        # Applied to both agents — the hunter should navigate around
        # obstacles rather than pressing into them, and the prey must keep
        # moving to evade rather than hiding in a corner.
        if self.hunter_stuck_steps > 0:
            rewards["hunter"] -= R_STUCK_SCALE * self.hunter_stuck_steps
        if self.prey_stuck_steps > 0:
            rewards["prey"]   -= R_STUCK_SCALE * self.prey_stuck_steps
