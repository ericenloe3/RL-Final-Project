"""
SAC training for 2-hunter / 1-prey hunter-prey (v3 — hunter coordination fix).

v2 results recap:
  - Capture peaked at 100% at step 755k, sustained ~3M steps, then
    collapsed to ~19% by step 10M.  Final eval: 40% capture rate (up
    from v1's 13% — improvement) but hunters still lose ~60% of episodes.
  - Eval trajectory analysis revealed a specific hunter failure mode:
    in many timeout episodes, hunters START in flanking configuration
    (flank score +0.86 at step 0), then converge to the SAME side of
    the prey within ~100 steps (flank score -0.88), then FREEZE for
    the rest of the episode (distances barely change for steps 100-600).
  - Root cause: the parameter-shared hunter network produces correlated
    actions for the two hunters because they have similar observations.
    Without explicit coordination signal, "spread to flank" is not in
    the policy gradient.  Once hunters cluster, the prey hides in the
    unwatched region; the search-reward cutoff at staleness=100 then
    leaves hunters with NO useful per-step gradient — flat-valley
    landscape, no incentive to do anything, hence the freeze.

v3 changes (env.py + this file):

  ENV.PY:
    1. NEW R_FLANK = 0.02 per step per hunter.  Fires when the angle
       between (prey→hunter1) and (prey→hunter2) is obtuse (hunters on
       opposite sides of prey).  Reward scales with -dot(v1, v2): max
       +0.02 when perfectly opposite, 0 when orthogonal, none when same
       side.  This forces the shared policy to learn "if my teammate
       is on the prey's left, I should approach from the right" — the
       coordination pattern that 2v1 needs to be effective.

    2. SEARCH-REWARD CUTOFF SMOOTHED.
       Old: search reward fires only when staleness < 100, then zero.
       Creates the flat-gradient cliff causing the freeze behavior.
       New: linear decay from 1.0 at staleness=0 to 0.0 at staleness=300.
       Always SOME gradient toward last-known prey position, never an
       abrupt zero.  Hunters keep moving even when prey is long-hidden.

  TRAIN_SAC.PY:
    3. BUFFER_SIZE 1M → 500k.
       The off-policy collapse (peak then crash) was driven by the
       hunter Q-function fitting an outdated prey distribution while
       the prey was actively learning to evade.  Halving the buffer
       size halves the average staleness of training samples, helping
       the critic track the current prey policy faster.  Trade-off: a
       bit less sample efficiency, but stability matters more than raw
       efficiency at this scale.

Hyperparameters retained from v2:
  - HIDDEN = 192/192, parameter-shared hunter network
  - α-floor via softplus reparam, asymmetric (hunter 0.05, prey 0.02)
  - α init at 0.2, target_entropy = 0.0
  - Critic warmup 5000 steps, UTD = 0.25
  - max_steps 600
  - Speed curriculum: 4.5/4.5 first half, 4.7/4.5 second half
...
TAU               = 0.005
BATCH_SIZE        = 256
BUFFER_SIZE       = 500_000      # CHANGED 1M → 500k for faster adaptation
